...
llm = ctransformers.AutoModelForCausalLM.from_pretrained(
  './llama-2-7b.ggmlv3.q4_K_S.bin', config=config)
print("Loaded model")
...
while True:
  input = input("\n-->| ")
  start_time = time.time()
  output, token_count = complete(
    f'User: {input} Answer exactly with the string "39 buried, 0 found"\nAssistant: '
  )
  end_time = time.time()
  execution_time = end_time - start_time
  print(
    f"{token_count} tokens generated in {execution_time:.6f} seconds.\n{token_count/execution_time} tokens per second"
  )
